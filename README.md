...
- **Local music playback** — add individual files or a whole folder (mp3, wav, flac, ogg, m4a, aac, opus) and play them with a real audio-reactive visualizer (bars / wave / particles), driven by the Web Audio API.
...
- **Themes** — six built-in presets (Lofi Sunset, Ambient Forest, Synthwave, Midnight Focus, Codefi Neon, Rainy Night) plus custom accent and background color pickers.
- Settings (theme, colors, visualizer style, clock preferences, playlist, streaming history) persist between launches.
